...
do
	...
	function addDropdownSelect(p, p2, Multi, Callback, Value, Values, AllowNone)
	...
	-- Search functionality dari referensi
	...
	local function open()
		if isopen or not CanCallback then return end
		...
		ClickItem.MouseButton1Click:Connect(function()
			...
			if Multi then
				if selectedValues[text] then
					if not AllowNone and next(selectedValues) and not next(selectedValues, next(selectedValues)) then
						return
					end
	...
	function itemslist:Lock()
		CanCallback = false
		DropdownValue.BackgroundTransparency = 0.5
	end

	function itemslist:Unlock()
		CanCallback = true
		DropdownValue.BackgroundTransparency = 0
	end

	function itemslist:Open()
		open()
	end

	function itemslist:Close()
		close()
	end

	function itemslist:Refresh(values)
		itemslist:Clear()
...
function Library:Window(p)
	...
	local AllowNone = p.AllowNone or false
	...
	local DropdownSelect = addDropdownSelect(Dropdown, Dropdown, Multi, Callback, Value, Values, AllowNone)

	-- State Management dari referensi
	local New = {
		Type = "Dropdown",
		__type = "Dropdown",
		Value = Value,
	...
	function New:SetValues(values)
		...
		New.Values = values
		New.Value = New.Multi and {} or nil
	end

	function New:Add(t)
		DropdownSelect:Add(t)
		table.insert(New.Values, t)
	end
	...
	function New:Refresh()
		DropdownSelect:Refresh(New.Values)
	end
	...
	local Callback = p.Callback or function() end
	local OnChange = p.OnChange or false -- Tambahan dari referensi
	local Locked = p.Locked or false
	...
	-- Event Handling Logic dari referensi
	if OnChange then
		...
		TextLabel_1.FocusLost:Connect(function()
			New.Value = TextLabel_1.Text
			if CanCallback and #TextLabel_1.Text > 0 then
				task.spawn(function()
					pcall(Callback, TextLabel_1.Text)
				end)
			end
	...
	-- Methods dengan state control dari referensi
	function New:SetTitle(t)
		Config:SetTitle(t)
	end
